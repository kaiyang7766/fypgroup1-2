...
if __name__ == '__main__':
 print('----------------------')
 print('\033[4mKetones\033[0m\t\033[4mPredicted Nucleophilicity\033[0m')
 mol_name_sorted=[]
 for i in range(len(mol_name)):
  #print(mol_name[i], float(y[:,i]))
  mol_name_sorted.append([mol_name[i],float(y[:,i])])
 mol_name_sorted.sort(key=lambda x:x[1],reverse=True)
 #print('Sorted list')
 for i in range(len(mol_name_sorted)):
     print(mol_name_sorted[i][0],mol_name_sorted[i][1])
